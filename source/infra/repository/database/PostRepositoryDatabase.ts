...
export default class PostRepositoryDatabase implements PostRepositoryInterface {
    ...
    async create(post: Post): Promise<Post | null> {
        ...
        await this.connection.execute("INSERT INTO posts (id, description, post_string, created_at, updated_at, deleted_at) values ($1, $2, $3);", [ post.id, post.description, post.post_string, post.created_at, post.updated_at, post.deleted_at ]);
